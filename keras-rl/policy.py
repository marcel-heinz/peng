...
class EpsGreedyQPolicy(Policy):
    ...
    def __init__(self, action_size,
                 eps=.1,
                 q_Injection = True, 
                 qInject_steps=1000000):
        super(EpsGreedyQPolicy, self).__init__()
        self.eps = eps
        self.qInject_steps = qInject_steps
        self.startSteps = qInject_steps
        self.action_size = action_size
        self.q_Injection = q_Injection
        
        self.q_inject_array = self.getQs(self.action_size)
        
        self.phi = 1.0 #probability of Past Policy -> 1-phi = probability of new policy
        self.phi_decay = 0.95
        
        self.past_prob = None
        
    def getQs(self,action_size):
        a = np.random.uniform(0.4,0.45,action_size) #pacman0.5-0.62#seaquest0.4-0.45
        return a
    ...
    def select_action(self, q_values):
        ...
        if self.q_Injection:#QInjection
            #print("Startet Q-Injection")
        
            if self.qInject_steps % 100 == 0:
                    self.past_prob = self.phi
                    #self.q_inject_array = self.getQs()
                    
            if self.qInject_steps % 10000 == 0:
                    #self.past_prob = self.phi
                    self.q_inject_array = self.getQs(self.action_size)

            if self.qInject_steps == self.startSteps:
                    self.past_prob = self.phi
                    #self.q_inject_array = self.getQs()
            #print(f"Past Prob STart:{self.past_prob}")

            if self.qInject_steps <0: #no injection
                #print("Injection Over")
                if np.random.uniform() < self.eps:
                    action = np.random.random_integers(0, nb_actions-1)
                else:
                    action = np.argmax(q_values)

            else: #q.injection for qInject_steps
                if np.random.uniform() < self.past_prob:  
                    #print(f"Inject Old: past prob: {self.past_prob}")
                    action = np.argmax(self.q_inject_array)
                else:

                    if np.random.uniform() < self.eps:
                        #print("Inject new: random")
                        action = np.random.random_integers(0, nb_actions-1)
                    else:
                        #print("Inject new: exploit")
                        action = np.argmax(q_values)

            self.past_prob=self.past_prob*self.phi_decay   
            self.qInject_steps-=1
        
        else:#NoQInjection
            #print("Startet Normal Mode")
            if np.random.uniform() < self.eps:
                        #print("Inject new: random")
                        action = np.random.random_integers(0, nb_actions-1)
            else:
